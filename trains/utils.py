...
def soft_clip_loss(preds, targs, temp=0.125, bi = True):
    clip_clip = (targs @ targs.T)/temp
    brain_clip = (preds @ targs.T)/temp
    loss = -(brain_clip.log_softmax(-1) * clip_clip.softmax(-1)).sum(-1).mean()
    if bi:
...
def unclip_recon(x, diffusion_engine, vector_suffix,
    ...
    with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.float16), diffusion_engine.ema_scope():
        z = torch.randn(num_samples,4,96,96).to("cuda:1") # starting noise, can change to VAE outputs of initial image for img2img
        ...
        for k in c:
            c[k], uc[k] = map(lambda y: y[k][:num_samples].to("cuda:1"), (c, uc))
        ...
        samples_z = diffusion_engine.sampler(denoiser, noised_z.to("cuda:1"), cond=c, uc=uc)
