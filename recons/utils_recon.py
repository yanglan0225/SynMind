...
def unclip_recon(x, diffusion_engine, vector_suffix,
                 num_samples=1, offset_noise_level=0.04,device = "cuda:0"):
    assert x.ndim==3
    if x.shape[0]==1:
        x = x[[0]]
    with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.float16), diffusion_engine.ema_scope():
        ...
        return samples
def append_dims_blur(x, target_dims):
    """Appends dims to the end of a tensor until it has target_dims."""
...
def unclip_recon_blur(x, diffusion_engine, vector_suffix,
                 num_samples=1, offset_noise_level=0.04,
                 blurry_image=None, strength=0.7):
    """
    Generates an image from a CLIP embedding, with an optional starting image.

    Args:
        x (torch.Tensor): The input CLIP embedding (from fMRI). Shape: [1, N, C].
        diffusion_engine: The loaded DiffusionEngine model.
        vector_suffix: The suffix vector for conditioning.
        num_samples (int): Number of samples to generate.
        offset_noise_level (float): Level of offset noise.
        blurry_image (torch.Tensor, optional): A preprocessed blurry image tensor to
            start from. Shape: [1, 3, H, W], range [0, 1]. If None, starts from
            pure noise. Defaults to None.
        strength (float, optional): Controls how much noise is added to the blurry
            image. 0.0 preserves the image perfectly, 1.0 is equivalent to
            starting from pure noise. Relevant only if blurry_image is provided.
            Defaults to 0.7.
    """
    assert x.ndim == 3
    if x.shape[0] == 1:
    ...
    with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.float16), diffusion_engine.ema_scope():
        # --- Conditioning setup (remains the same) ---
        blurry_image_norm = blurry_image * 2.0 - 1.0
        z = diffusion_engine.encode_first_stage(blurry_image_norm.to(dtype=torch.float32))
        ...
        # --- Initial Latent (z) and Noise setup ---
        sigmas = diffusion_engine.sampler.discretization(diffusion_engine.sampler.num_steps)

        if blurry_image is not None:
            # --- IMAGE-TO-IMAGE MODE ---
            print(f"Running in Image-to-Image mode with strength: {strength}")
            if not (0.0 < strength <= 1.0):
                raise ValueError("Strength must be in (0, 1]")

            # 1. Encode the blurry image into the latent space
            # The VAE expects input in [-1, 1], so we transform from [0, 1] to [-1, 1]
            blurry_image_norm = blurry_image * 2.0 - 1.0
            z0 = diffusion_engine.encode_first_stage(blurry_image_norm.to(dtype=torch.float32))
            print(z0.shape)

            # 2. Determine the starting timestep based on strength
            # A strength of 1.0 means we start from the very beginning (max noise)
            # A strength of 0.7 means we skip the first 30% of noise steps
            num_steps = diffusion_engine.sampler.num_steps
            start_step_idx = num_steps - int(num_steps * strength)
            
            # Make sure we don't go out of bounds
            start_step_idx = max(0, start_step_idx)
            
            # 3. Get the sigma for the starting timestep
            sigma = sigmas[start_step_idx].to("cuda")

            # 4. Add noise to the encoded latent
            noise = torch.randn_like(z0)
            noised_z = z0 + noise * sigma

            # The sampler needs to know which sigmas to use for denoising.
            # We pass only the relevant part of the schedule.
            sampler_sigmas = sigmas[start_step_idx:]

        else:
            # --- TEXT-TO-IMAGE MODE (Original Code) ---
            print("Running in Text-to-Image mode (from pure noise)")
            z = torch.randn(num_samples, 4, 96, 96).to("cuda")
            sigma = sigmas[0].to(z.device)
            noise = torch.randn_like(z)
            
            if offset_noise_level > 0.0:
                noise = noise + offset_noise_level * append_dims_blur(
                    torch.randn(z.shape[0], device=z.device), z.ndim
                )
            
            noised_z = z + noise * append_dims_blur(sigma, z.ndim)
            noised_z = noised_z / torch.sqrt(1.0 + sigma ** 2.0)
            sampler_sigmas = sigmas

        # --- Denoising and Decoding (modified sampler call) ---
        def denoiser(x, sigma, c):
            return diffusion_engine.denoiser(diffusion_engine.model, x, sigma, c)
        
        # The sampler needs to be passed the correct (potentially truncated) sigma schedule
        # NOTE: This assumes your sampler can accept a `sigmas` argument. Most SGM/Diffusers samplers do.
        # If your sampler implementation is different, this part may need adjustment.
        samples_z = diffusion_engine.sampler(denoiser, noised_z, cond=c, uc=uc)
