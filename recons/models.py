...
class sem_brainMLP(nn.Module):
    ...
    def __init__(self,hidden_dim=4096):
        ...
        self.layer4 = nn.Linear(hidden_dim, 77*768)  # 输出层
...
class BrainNetwork(nn.Module):
    ...
    def __init__(self, h=4096, in_dim=15724, out_dim=768, seq_len=2, n_blocks=4, drop=.15, clip_size=768, blurry_recon=True, clip_scale=1,text_out_dim = 1280*77, use_text = True):
        ...
        if use_text:
            self.text_mixer_blocks1 = nn.ModuleList([
                self.mixer_block1(h, drop) for _ in range(n_blocks)
            ])
            self.text_backbone_linear = nn.Linear(h * seq_len, text_out_dim, bias=True) 
    ...
    def forward(self, x):
        ...
        text_backbone = torch.Tensor([0.])
        text_emb = torch.Tensor([0.])
        ...
        if self.use_text:
            for block1, block2 in zip(self.text_mixer_blocks1,self.mixer_blocks2):
                text_x = block1(text_x) + residualtext
                residual1 = text_x

            text_backbone = self.text_backbone_linear(text_x).reshape(len(x), -1, 768)
